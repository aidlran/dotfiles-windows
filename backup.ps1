...
#{
    # Full backup
    #7zip a -p -mhe=on -mx9 -mmt12 -t7z -spf "\\NAS\Backup\pc\$dateMonth.7z" "C:\" -xr@"$env:USERPROFILE\exclude.txt"
    7zip a -p -mhe=on -mx9 -mmt12 -t7z -spf "\\NAS\Backup\pc\$dateFull.7z" "C:\" -xr@"$env:USERPROFILE\exclude.txt"
#}
